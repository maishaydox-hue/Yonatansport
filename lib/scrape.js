...
async function fetchPlayerGamesFragment(playerId, seasonId) {
  ...
  const commonHeaders = {
    ...
    Referer: `https://www.football.org.il/players/player/?player_id=${playerId}${seasonId ? `&season_id=${seasonId}` : ''}`,
  };
  ...
  async function tryParseJson(res) {
    ...
    try {
      ...
      throw new Error('לא JSON: ' + raw.slice(0, 120));
    }
  }

  // Attempt 1: GET with AJAX-style headers (matches what the browser sent).
  try {
    ...
    if (res.ok) {
      ...
      if (json && json.d) return json.d;
    }
  } catch (e) {
    // fall through to attempt 2
  }

  // Attempt 2: some ASP.NET ScriptServices only return JSON for POST.
  const res2 = await fetch(url.toString(), {
    method: 'POST',
    headers: { ...commonHeaders, 'Content-Type': 'application/json; charset=UTF-8' },
    body: '{}',
  });
  if (!res2.ok) throw new Error(`GetPlayerGames נכשל (סטטוס ${res2.status})`);
  const raw2 = await res2.text();
  try {
    const json2 = JSON.parse(raw2);
    return json2 && json2.d ? json2.d : '';
  } catch (e) {
    throw new Error('GetPlayerGames החזיר תשובה לא צפויה גם ב-GET וגם ב-POST: ' + raw2.slice(0, 150));
  }
}
