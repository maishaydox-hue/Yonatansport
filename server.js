...
app.post('/api/weekly-stats/upload', async (req, res) => {
  try {
    ...
    if (type === 'image' || type === 'pdf') {
      ...
      const expectedPrefix = type === 'pdf' ? 'data:application/pdf' : 'data:image';
      if (!dataUrl || !dataUrl.startsWith('data:') || (type === 'pdf' && !dataUrl.startsWith(expectedPrefix))) {
        return sendError(res, 400, type === 'pdf' ? 'קובץ PDF לא תקין התקבל.' : 'תמונה לא תקינה התקבלה.');
      }

      const match = dataUrl.match(/^data:(.+?);base64,(.+)$/);
      if (!match) {
        return sendError(res, 400, 'לא ניתן היה לפענח את הקובץ.');
      }
      const [, mime, base64] = match;
      const buffer = Buffer.from(base64, 'base64');
      const ext = (mime.split('/')[1] || (type === 'pdf' ? 'pdf' : 'jpg')).replace(/[^a-z0-9]/gi, '');
      ...
      summary.latestSource = { id: inserted.id, url: inserted.image_url, fileName: inserted.file_name, type };
